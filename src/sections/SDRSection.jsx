...
import { 
  Users, Target, Rocket, Briefcase, ChevronRight, 
  Check, Sparkles, LayoutGrid, Globe, Zap, 
  BarChart3, MousePointer2, ShieldCheck, Cpu,
  CalendarCheck, User, Bot, Mail, Send, MoreHorizontal,
  Clock, Activity, PieChart, TrendingUp, Filter, CheckCircle2,
  FileText, ArrowRight, BarChart
} from 'lucide-react';
  ...
  return (
    <div className='relative h-full w-full p-6 flex flex-col justify-center bg-[#050505]'>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                <div className="text-[11px] font-bold text-white tracking-tight">{client.name}</div>
                <div className="flex items-center gap-1.5">
                    <div className="w-1 h-1 rounded-full bg-indigo-500 animate-pulse" />
                    <div className="text-[8px] text-zinc-500 uppercase tracking-widest">{client.agents} Active Agents</div>
                </div>
              </div>
            </div>
      ...
      <div className="absolute bottom-4 left-6 flex items-center gap-2">
         ...
         <span className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">Multi-Tenant Orchestration</span>
      </div>
    </div>
  );
}
      ...
      <div className="relative w-full max-w-[240px] space-y-6 z-10">
        <div className="flex items-center justify-between px-2">
            <div className="flex flex-col gap-1">
                <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest">Ingested Signals</span>
                <span className="text-sm font-bold text-white tracking-tighter">842 / hour</span>
            </div>
        ...
        <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Automatic Quota Fill</span>
            </div>
    ...
    <div className='relative h-full w-full flex flex-col items-center justify-center p-8 bg-[#050505]'>
       <div className="flex flex-col gap-3 w-full max-w-[200px] relative z-10">
            <div className="p-3 rounded-xl bg-zinc-900 border border-white/10 flex items-center justify-between opacity-50">
                <span className="text-[10px] text-zinc-400 font-medium">Manual Prospecting</span>
                <span className="text-[9px] font-bold text-rose-500 uppercase">Offloaded</span>
            </div>
            <div className="p-3 rounded-xl bg-zinc-900 border border-white/10 flex items-center justify-between opacity-50">
                <span className="text-[10px] text-zinc-400 font-medium">CRM Data Entry</span>
                <span className="text-[9px] font-bold text-rose-500 uppercase">Offloaded</span>
            </div>
            <motion.div 
            ...
            >
                <span className="text-[10px] text-indigo-200 font-bold">Scaling Growth</span>
                <span className="text-[9px] font-bold text-indigo-400 uppercase animate-pulse">Running</span>
            </motion.div>
       </div>
...
const UseCaseCard = ({ name, outcome, description, visual: Visual, icon: Icon, details, delay }) => {
            ...
            <div className="relative z-20 p-10 flex flex-col flex-1 bg-[#0a0a0a]">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-3xl font-bold text-white tracking-tighter group-hover:text-indigo-200 transition-colors leading-tight">{name}</h3>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="w-1 h-1 rounded-full bg-emerald-500" />
                                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">{outcome}</span>
                            </div>
                        </div>
                    </div>
                </div>
                
                <p className="text-zinc-400 text-sm leading-relaxed mb-8 font-light min-h-[48px]">{description}</p>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2">
                        {details.map((tag, i) => (
                            <div key={i} className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{tag}</span>
                            </div>
                        ))}
                    </div>
    ...
    <section className="py-32 px-6 bg-[#020204] relative overflow-hidden font-sans selection:bg-indigo-500/30">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-28 flex flex-col items-center">
          <ScrollReveal>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[11px] font-bold uppercase tracking-widest text-zinc-400 mb-8">
                <Sparkles size={12} className="text-indigo-400" />
                Select Your Operating Motion
              </div>
          </ScrollReveal>
          
          <h2 className="text-4xl md:text-6xl  lg:text-6xl font-bold text-white tracking-tighter mb-8 leading-[0.85] max-w-4xl">
            Built for the <br /> 
            <span className="bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-zinc-600">New Sales Standard.</span>
          </h2>
          <p className="text-zinc-400 max-w-2xl mx-auto text-xl font-light leading-relaxed">
            One engine, three ways to dominate. Automated outbound for the most ambitious growth teams.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <UseCaseCard 
                ...
                icon={Globe}
                visual={AgencyVisual}
                details={['70% Margin Boost', 'White-Label Ready', 'Multi-Tenant HUD']}
            />
            <UseCaseCard 
                delay={200}
                name="Sales Teams"
        ...
        <div className="mt-32 pt-16 border-t border-white/5 flex flex-col lg:flex-row items-center justify-between gap-12">
             <div className="flex items-center gap-8 group">
                ...
                <div className="text-left">
                    <div className="text-xs font-bold text-white uppercase tracking-[0.2em] mb-1">Bank-Grade Infrastructure</div>
                    <div className="text-sm text-zinc-500 font-light max-w-[340px]">Nexuscale is SOC2 Type II compliant and utilizes AES-256 encryption. Your client data stays private and secure.</div>
                </div>
             </div>
             
             <div className="flex gap-16 md:gap-24">
                <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] mb-1">Daily Signals</span>
                    <div className="text-4xl font-bold text-white tracking-tighter">100k+ <span className="text-indigo-500/80 italic font-medium">processed</span></div>
                </div>
                <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] mb-1">Active Agents</span>
                    <div className="text-4xl font-bold text-white tracking-tighter">2.4k <span className="text-purple-500/80 italic font-medium">live</span></div>
                </div>
             </div>
        </div>
