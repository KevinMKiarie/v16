  ...
  return (
    ...
    >
      <Ripple
        ...
        numCircles={4}
        className="mask-[linear-gradient(to_bottom,white_50%,transparent)]"
      />

      <div
        ref={topLeftRef}
        className="absolute top-3 left-5 w-7 h-7 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-[9px] font-bold text-emerald-400"
      >
        $
      </div>
      <div
        ref={topRightRef}
        className="absolute top-3 right-5 w-7 h-7 rounded-full bg-blue-500/20 border border-blue-500/30 flex items-center justify-center text-[9px] font-bold text-blue-400"
      >
        H
      </div>
      <div
        ref={bottomLeftRef}
        className="absolute bottom-3 left-5 w-7 h-7 rounded-full bg-violet-500/20 border border-violet-500/30 flex items-center justify-center text-[9px] font-bold text-violet-400"
      >
        T
      </div>
      <div
        ref={bottomRightRef}
        className="absolute bottom-3 right-5 w-7 h-7 rounded-full bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-[9px] font-bold text-amber-400"
      >
        R
      </div>
      ...
      <div
        ref={centerRef}
        className="relative z-10 w-10 h-10 rounded-full bg-blue-600/30 border border-blue-500/40 flex items-center justify-center shadow-[0_0_25px_rgba(59,130,246,0.3)]"
      >
    ...
    <div className="relative w-full h-full flex flex-col gap-2 p-4 overflow-hidden">
      <div className="relative z-10 space-y-2">
        ...
        <div className="bg-white/4 border border-white/8 rounded-lg p-2.5 text-[11px] text-zinc-300 leading-relaxed">
          ...
          <span className="text-indigo-400 bg-indigo-500/10 px-1 rounded font-medium">
            Head of AI
          </span>
          ! With TechFlow&apos;s recent{" "}
          <span className="text-emerald-400 bg-emerald-500/10 px-1 rounded font-medium">
            Series A
          </span>
          , I imagine you&apos;re scaling fast...
        </div>
        <div className="flex gap-1.5 flex-wrap">
          <span className="text-[8px] bg-indigo-500/10 text-indigo-300 px-1.5 py-0.5 rounded-full border border-indigo-500/20">
            Role Change
          </span>
          <span className="text-[8px] bg-emerald-500/10 text-emerald-300 px-1.5 py-0.5 rounded-full border border-emerald-500/20">
            Funding
          </span>
          <span className="text-[8px] bg-amber-500/10 text-amber-300 px-1.5 py-0.5 rounded-full border border-amber-500/20">
            Hiring
          </span>
        </div>
      </div>
...
function BookingVisual() {
  ...
  return (
    ...
    >
      <div
        ref={autopilotRef}
        className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-blue-600/25 border border-blue-500/40 flex items-center justify-center shadow-[0_0_15px_rgba(59,130,246,0.2)]"
      >
      ...
      <div className="flex flex-col gap-3">
        <div
          ref={emailRef}
          className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-[11px]"
        >
          📧
        </div>
        <div
      ...
      <div
        ref={calendarRef}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 rounded-lg bg-emerald-600/20 border border-emerald-500/30 flex items-center justify-center"
      >
              ...
              return (
                ...
                >
                  <div className="relative h-full overflow-hidden rounded-2xl border border-white/8 bg-[#09090B] group hover:border-white/15 transition-colors">
                    ...
                    <div
                      className={`flex h-full ${i === 2 ? "flex-col md:flex-row" : "flex-col"}`}
                    >
                      {/* Visual area */}
                      <div
                        className={`relative bg-white/[0.02] border-b border-white/6 ${
                          i === 2
                            ? "h-48 md:h-auto md:w-1/2 md:border-b-0 md:border-r"
                            : "h-44"
                        } overflow-hidden`}
                      >
                        <power.visual />
                      ...
                      {/* Content area */}
                      <div
                        className={`flex flex-col gap-4 p-5 md:p-6 ${i === 2 ? "md:w-1/2 md:justify-center" : ""}`}
                      >
