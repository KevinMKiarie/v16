    ...
    <div className="pt-24 min-h-screen animate-in fade-in slide-in-from-bottom-8 duration-400">
      <section className="relative py-24 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div
            className={`inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-indigo-500/10 text-indigo-400 mb-8 border border-indigo-500/20 shadow-[0_0_50px_-10px_rgba(99,102,241,0.4)] backdrop-blur-xl ${neu.surface}`}
          >
            {typeof Icon === "string" ? (
              <img
                src={Icon}
      ...
      <section className="py-24 px-6 border-y border-white/5 bg-black/20 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
          {feature.benefits?.map((benefit, i) => (
            <SpotlightCard
              key={i}
              className="p-8 rounded-3xl h-full flex flex-col items-center text-center backdrop-blur-md border-white/10 hover:border-indigo-500/30"
            >
              ...
              <h3 className="text-white font-bold mb-4 text-xl flex items-center gap-2 justify-center tracking-tight">
                {benefit.title}
                {benefit.soon && (
                  <span className="text-[10px] bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 rounded uppercase tracking-wide font-bold">
                    Soon
                  </span>
                )}
              </h3>
