    ...
    <div className="pt-24 min-h-screen animate-in fade-in slide-in-from-bottom-8 duration-400">
      <section className="relative py-24 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div
            className={`inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-white/[0.03] text-white mb-8 border border-white/10 shadow-2xl backdrop-blur-xl ${neu.surface}`}
          >
            <Icon className="w-12 h-12" />
          </div>
          <div className="text-indigo-400 font-bold uppercase tracking-[0.3em] mb-6 text-sm bg-indigo-500/10 px-4 py-1.5 rounded-full inline-block border border-indigo-500/20">
            Built For
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold text-white mb-8 tracking-tight drop-shadow-2xl">
            {useCase.title}
          </h1>
          <p className="text-2xl text-zinc-300 max-w-2xl mx-auto leading-relaxed mb-12 font-medium">
            {useCase.desc}
          </p>
      ...
      <section className="py-24 px-6 border-y border-white/5 bg-black/20 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
          {useCase.benefits?.map((benefit, i) => (
            <SpotlightCard
              key={i}
              className="p-10 rounded-3xl h-full flex flex-col hover:border-indigo-500/30 group"
            >
              ...
              <h3 className="text-white font-bold mb-4 text-2xl tracking-tight">
                {benefit.title}
              </h3>
