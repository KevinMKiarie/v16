...
export default function CaseStudyDetailsPage() {
  ...
  useEffect(() => {
    ...
    const description = (
      ...
      .slice(0, 160);
    const author =
      typeof study.author === "string"
        ? study.author
        : study.author?.name || "Nexuscale";
    const updatedAt =
      study.updated_at || study.inserted_at || study.published_at;
    ...
    getOrCreateMetaTag("description").setAttribute(
      "content",
      description || `${title} - Nexuscale Case Study`,
    );
    getOrCreateMetaTag("og:title", true).setAttribute("content", title);
    getOrCreateMetaTag("og:description", true).setAttribute(
      "content",
      description || `${title} - Nexuscale Case Study`,
    );
    ...
    getOrCreateMetaTag("og:site_name", true).setAttribute(
      "content",
      "Nexuscale",
    );
    getOrCreateMetaTag("twitter:card").setAttribute(
      "content",
      "summary_large_image",
    );
    getOrCreateMetaTag("twitter:title").setAttribute("content", title);
    getOrCreateMetaTag("twitter:description").setAttribute(
      "content",
      description || `${title} - Nexuscale Case Study`,
    );
    ...
    if (updatedAt) {
      getOrCreateMetaTag("article:published_time").setAttribute(
        "content",
        new Date(updatedAt).toISOString(),
      );
      getOrCreateMetaTag("article:modified_time").setAttribute(
        "content",
        new Date(updatedAt).toISOString(),
      );
    }
    if (author)
      getOrCreateMetaTag("article:author").setAttribute("content", author);
    ...
    if (imageUrl)
      jsonLd.image = imageUrl.startsWith("http")
        ? imageUrl
        : `https://articles.nexuscale.ai${imageUrl}`;
